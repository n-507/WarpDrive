		...
		if ( blockEvent instanceof BlockEvent.MultiPlaceEvent
		  || blockEvent instanceof BlockEvent.EntityMultiPlaceEvent ) {
			...
			for (final BlockSnapshot blockSnapshot : listBlockSnapshots) {
				...
				isAllowed = isAllowed && StarMapRegistry.onBlockUpdating(entity, blockEvent.getWorld(), blockSnapshot.getPos(), blockStateCurrent);
				if (blockStateCurrent != blockSnapshot.getReplacedBlock()) {
					isAllowed = isAllowed && StarMapRegistry.onBlockUpdating(entity, blockEvent.getWorld(), blockSnapshot.getPos(), blockSnapshot.getReplacedBlock());
				}
			}
			...
			isAllowed = isAllowed && StarMapRegistry.onBlockUpdating(entity, blockEvent.getWorld(), blockEvent.getPos(), blockEvent.getState());
		}
		if (!isAllowed) {
